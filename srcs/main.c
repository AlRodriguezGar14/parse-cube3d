...
/*   Updated: 2024/06/12 03:46:50 by dgomez-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_game_terminal(t_cube_data *cube_data)
{
	...
	while (cube_data->map[++ydx])
	{
		...
		{
			if (ydx == cube_data->player_position->y
				&& xdx == cube_data->player_position->x)
				printf("🦀");
			else
			{
...
int	main(int argc, char **argv)
{
	...
	if (read_file(argv[1], &cube_data) == 1)
		return (1);
	replace_tabs_with_spaces(&cube_data.map);
		// this is a way to "justify" the map and avoid spacing issues
